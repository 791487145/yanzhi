...
class MenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $result     = Db::name('admin_menu_1')->order(["list_order" => "ASC"])->select()->toArray();
        $tree       = new Tree();
        $tree->icon = ['&nbsp;&nbsp;&nbsp;│ ', '&nbsp;&nbsp;&nbsp;├─ ', '&nbsp;&nbsp;&nbsp;└─ '];
        $tree->nbsp = '&nbsp;&nbsp;&nbsp;';
    ...
    public function lists()
    {
        ...
        $result = Db::name('admin_menu_1')->order(["app" => "ASC", "controller" => "ASC", "action" => "ASC"])->select();
    ...
    public function add()
    {
        ...
        $result   = Db::name('admin_menu_1')->order(["list_order" => "ASC"])->select();
        $array    = [];
        foreach ($result as $r) {
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                Db::name('admin_menu_1')->strict(false)->field(true)->insert($data);
    ...
    public function edit()
    {
        ...
        $rs     = Db::name('admin_menu_1')->where(["id" => $id])->find();
        $result = Db::name('admin_menu_1')->order(["list_order" => "ASC"])->select();
        $array  = [];
        foreach ($result as $r) {
    ...
    public function editPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                Db::name('admin_menu_1')->strict(false)->field(true)->update($this->request->param());
                $app          = $this->request->param("app");
                $controller   = $this->request->param("controller");
                $action       = $this->request->param("action");
                $param        = $this->request->param("param");
    ...
    {
        $id    = $this->request->param("id", 0, 'intval');
        $count = Db::name('admin_menu_1')->where(["parent_id" => $id])->count();
        if ($count > 0) {
            $this->error("该菜单下还有子菜单，无法删除！");
        }
        if (Db::name('admin_menu_1')->delete($id) !== false) {
            $this->success("删除菜单成功！");
        } else {
            $this->error("删除失败！");
        }
                ...
                if (!empty($menuAnnotations)) {
                    foreach ($menuAnnotations as $menuAnnotation) {
                        ...
                        if (empty($menuAnnotation->parent)) {
                            ...
                            $findParentAdminMenu = Db::name('admin_menu_1')->where([
                                'app'        => $parentApp,
                            ...
                            if (empty($findParentAdminMenu)) {
                                $parentId = Db::name('admin_menu_1')->insertGetId([
                                    'app'        => $parentApp,
                                    'controller' => $parentController,
                                    'action'     => $parentAction,
                                    'name'       => '--new--'
                        ...
                        $findAdminMenu = Db::name('admin_menu_1')->where([
                            'app'        => $app,
    ...
    {
        $menus         = Db::name('admin_menu_1')->order(["app" => "ASC", "controller" => "ASC", "action" => "ASC"])->select();
