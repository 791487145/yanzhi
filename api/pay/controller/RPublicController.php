...
class RPublicController extends RestBaseController
{
    private $ratio = 10;
    ...
    private function updateData($order , $update) {
        ...
        if ( $order['channel_statis'] == 1 ) {//需要处理推广统计的，获取提成信息
            $channel = Db::name('channel')->where(['user_id'=>$user['channel_id']])->find();
            if (!$channel ){//平台用户推广
                ...
                $update['ratio_parent']          = $order['type'] == 1 ? $channel['ratio_paren'] : $channel['ratio_vip_parent'];   //渠道的上级提成比例
                if ($channel['effective'] < 100) {//有效订单比例<100,表示有扣量订单
                    $count = Db::name("pay_payment")
                        ->field("count(id) pay_num,sum(channel_statis) sta_num")->where('channel_id',$user['channel_id'])->where('status',1)
