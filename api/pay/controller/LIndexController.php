...
class LIndexController extends RestUserBaseController
{
    private $ratio = 10;

    public function payBeiweiOrder(OrderService $orderService,Request $request)
    {
        $user = $this->user;
        ...
        $orderService->order($user,$data);
        halt(1);
        $type = $this->request->post('type', 0, 'intval');//
        $mode = $this->request->post('mode', 1);//支付方式1:alipay;2wechat
        $money = $this->request->post('money', 0, 'intval');//

        if ($money == 0)$this->error("请选择充值金额");

        $reOrder = PayPaymentModel::where(['user_id'=>$user['id']])->where('status',0)->where('type',$type)->where('pay_mode',$mode)->where('money',$money)->order('id desc')->find();
