...
class Captcha
{
    ...
    public function entry($id = '')
    {
        ...
        // 验证码使用随机字体
        $ttfPath = __DIR__ . '/../assets/' . (($this->useZh || $this->useNum) ? 'zhttfs' : 'ttfs') . '/';
        ...
        if ($this->useZh) {
            ...
            for ($i = 0; $i < 5; $i++) {
                imagettftext($this->_image, $this->fontSize, mt_rand(-20, 20), $this->fontSize * ($i + 1) * 1.5, $this->fontSize + mt_rand(0, 10), $this->_color, $this->fontttf, $code[$i]);
            }
            $code                  = $this->authcode(strtoupper($code[0] + $code[2]));
        } else {
            for ($i = 0; $i < $this->length; $i++) {
